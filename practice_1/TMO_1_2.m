...
for i = 1:length(N_array)
    ...
    % Генерация случайных величин на интервале [0.1;10]
    u = rand(N, 1);  % u - случайные величины на интервале [0, 1]
    x = 10.^(u * (log10(10) - log10(0.1)) + log10(0.1));  % пересчет x
...
for i = 1:length(N_array)
    ...
    for j = 1:length(alpha_values)
        ...
        interval_means(i, j, :) = [mean_x - margin_mean, mean_x + margin_mean];
...
for i = 1:length(alpha_values)
    ...
    uitable('Data', mean_intervals_data{:,:}, 'ColumnName', mean_intervals_data.Properties.VariableNames, ...
        'RowName', [], 'Position', [20 20 500 150]);
